...
function generateAIPrompt(csvData: string): string {
  return You are a data analyst expert. Analyze the following trend data and provide insights in simple, clear language.
...
Write in clear, simple language that a business person can understand. Use Hebrew for the response.
  ...
  try {
    ...
    if (!aiApiKey) {
      return NextResponse.json({
        csvData,
        prompt,
        analysis: To get AI analysis, please configure an AI API key (HUGGINGFACE_API_KEY or OPENAI_API_KEY) in your environment variables.
    ...
    try {
      ...
      if (response.ok) {
        ...
        return NextResponse.json({
          csvData,
          prompt,
          analysis: result[0]?.generated_text || result.generated_text || 'Analysis complete',
          needsApiKey: false
        });
      }
    } catch (hfError) {
