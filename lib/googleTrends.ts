...
export async function fetchGoogleShoppingTrends(
  ...
  // SearchAPI uses gprop=froogle for Google Shopping data (shopping is not a valid gprop)
  const result = await fetchGoogleTrends(keywords, timeParam, region, 'froogle', category);
  if (result) {
    result.label = 'Google Shopping';
    result.source = 'google_shopping';
    if (result.extra?.description) {
      result.extra.description = result.extra.description.replace('Froogle', 'Shopping');
    }
  }
  return result;
}
